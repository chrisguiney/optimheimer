...
static bool oph_app_is_vk_device_suitable(struct oph_application *app,
...
oph_app_select_vk_surface_format(struct oph_application *app,
                                 struct oph_vk_physical_device *dev)
{
...
static VkExtent2D
oph_app_get_vk_extent(struct oph_application *app,
                      const VkSurfaceCapabilitiesKHR *capabilities)
{
    ...
    VkExtent2D extent
        = {.width = clamp_uint32(app->presentation.framebuffer_width,
                                 capabilities->minImageExtent.width,
                                 capabilities->maxImageExtent.width),

           .height = clamp_uint32(app->presentation.framebuffer_height,
...
static void oph_app_init_vk_physical_device(struct oph_application *app,
                                            ...
                                            struct oph_vk_physical_device *dev)
{
    ...
    dev->surface_formats = oph_calloc(
        &app->sys, dev->n_surface_formats, sizeof(VkSurfaceFormatKHR));
    ...
    dev->present_modes = oph_calloc(
        &app->sys, dev->n_present_modes, sizeof(*dev->present_modes));

    vkGetPhysicalDeviceSurfacePresentModesKHR(
        dev->handle, dev->surface, &dev->n_present_modes, dev->present_modes);

    VkResult res = glfwCreateWindowSurface(app->backend.instance,
    ...
    dev->extension_properties = oph_calloc(&app->sys,
    ...
    dev->queue_families = oph_calloc(
        &app->sys, dev->n_queue_families, sizeof(*dev->queue_families));
...
static void oph_app_init_vk_physical_devices(struct oph_application *app)
{
    ...
    vkEnumeratePhysicalDevices(vk->instance, &vk->n_physical_devices, nullptr);
    ...
    for(size_t i = 0; i < app->backend.n_physical_devices; ++i)
    {
        oph_app_init_vk_physical_device(
            app, vk->vk_physical_devices[i], &vk->physical_devices[i]);

        if(oph_app_is_vk_device_suitable(app, &vk->physical_devices[i]))
        {
...
static void oph_app_init_vk_logical_devices(struct oph_application *app,
                                            struct oph_vk_physical_device *dev,
...
static void oph_app_init_presentation(const struct oph_configuration *config,
                                      struct oph_application *app)
{
    app->presentation.window = glfwCreateWindow(config->initial_window_width,
    ...
    glfwGetFramebufferSize(app->presentation.window,
                           &app->presentation.framebuffer_width,
                           &app->presentation.framebuffer_height);
}

static void oph_app_init_vk_imageview(struct oph_vk_logical_device *ldev,
...
static void oph_app_init_vk_swapchain(struct oph_application *app)
{
    app->swapchain.extent = oph_app_get_vk_extent(
        app, &app->backend.physical_device->surface_capabilities);

    app->swapchain.surface_format
        = oph_app_select_vk_surface_format(app, app->backend.physical_device);
    ...
    create_info.minImageCount
        = app->backend.physical_device->surface_capabilities.minImageCount;
    ...
    create_info.preTransform
        = app->backend.physical_device->surface_capabilities.currentTransform;
    ...
    VkResult res = vkCreateSwapchainKHR(app->backend.logical_device.handle,
                                        &create_info,
                                        nullptr,
    ...
    res = vkGetSwapchainImagesKHR(app->backend.logical_device.handle,
                                  app->swapchain.handle,
    ...
    app->swapchain.images = oph_calloc(
        &app->sys, app->swapchain.n_images, sizeof(*app->swapchain.images));
    ...
    for(size_t i = 0; i < app->swapchain.n_images; ++i)
    {
        oph_app_init_vk_imageview(
            &app->backend.logical_device, &app->swapchain, i);
    }
}
                  ...
                  struct oph_application *app)
{
    ...
    VkResult res
        = vkCreateInstance(&create_info, nullptr, &app->backend.instance);
    if(res)
    {
        abort();
    }
    ...
    oph_app_init_vk_logical_devices(
        app, app->backend.physical_device, &app->backend.logical_device);
